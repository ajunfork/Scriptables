...
class Widget extends Base {
  ...
  async renderMedium (count = 4) {
    ...
    let data = rst
    let w = new ListWidget()
    w = await this.renderHeader(w, 'https://cdn.pixabay.com/photo/2014/12/21/23/28/recipe-575434_960_720.png', '娟&俊食谱')
    ...
    for (let i = 0; i < count; i ++) {
      ...
      let pic = dom.addImage(await this.getImageByUrl("https://simg.s.weibo.com/20170303_img_search_"+(i+1)+".png"))
      pic.imageSize = new Size(18, 18)
      dom.addSpacer(5)
      let title = dom.addText(topic['pageName'])
      title.lineLimit = 1
      ...
      // }
      dom.addSpacer()
      // let extr = dom.addText(String(topic['openUrl'])) 
      // extr.font = Font.lightSystemFont(12)
      // extr.textOpacity = 0.6
      console.log(topic['openUrl'])
      dom.url = this.actionUrl('open-url', topic['openUrl'])
